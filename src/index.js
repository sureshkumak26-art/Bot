...
client.on('interactionCreate',async i=>{
 ...
 try{
  ...
  if(i.commandName==='profile'){const u=db.users&&db.users[i.user.id] ? db.users[i.user.id] : {orders:0};return i.reply(`👤 **${i.user.username}**\nOrders: **${u.orders||0}**\nWarnings: **${db.warnings[i.user.id] ? db.warnings[i.user.id].length : 0}**`);}
