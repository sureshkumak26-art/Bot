...
const commands = [
  new SlashCommandBuilder().setName('invoice').setDescription('Create a professional Anime Cloud invoice')
    ...
    .addStringOption(o => o.setName('items').setDescription('Items: Name=Price, Name=Price').setRequired(true))
    .addStringOption(o => o.setName('time').setDescription('Billing/service time, e.g. 30 Days').setRequired(true))
...
function parseItems(raw) {
  return raw.split(',').map(x => x.trim()).filter(Boolean).map(x => {
    ...
    if (i < 1) throw new Error('Items must use Name=Price format. Example: Anime VPS=750, Domain=1200');
    const name = x.slice(0, i).trim();
    const price = Number(x.slice(i + 1).replace(/[^0-9.]/g, ''));
    if (!name || !Number.isFinite(price)) throw new Error('Invalid item price.');
    return { name, price };
  });
}
...
async function makeInvoiceImage({ customer, items, time, status, number, imagePath }) {
