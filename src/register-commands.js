...
const commands = [
  ...
  new SlashCommandBuilder().setName('admin').setDescription('Anime Cloud admin controls')
    ...
    .addSubcommand(s => s.setName('broadcast').setDescription('Broadcast to this channel')
      .addStringOption(o => o.setName('message').setDescription('Message').setRequired(true)))
    .addSubcommand(s => s.setName('coupon').setDescription('Create coupon')
      .addStringOption(o => o.setName('code').setDescription('Code').setRequired(true))
      .addIntegerOption(o => o.setName('percent').setDescription('Discount percent').setMinValue(1).setMaxValue(100).setRequired(true)))
...
async function main() {
  ...
  console.log(`☁️ Anime Cloud: refreshed ${result.length} guild slash commands instantly.`);
}

main().catch(error => {
  console.error('❌ Guild command refresh failed:', error.message);
  process.exit(1);
});
